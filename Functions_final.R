...
# Function to compute the integral of the trace of the covariance function over [a, b]
compute_integral_trace <- function(a, b, covariance_function) {
  
  # Function to compute trace at given t
  trace_function <- function(t) {
    Sigma_t <- covariance_function(t)  # Compute covariance matrix at t
    return(sum(diag(Sigma_t)))  # Extract trace (sum of diagonal elements)
  }
  
  # Perform numerical integration over [a, b]
  integral_trace <- integrate(trace_function, lower = a, upper = b)$value
  
  return(integral_trace)
}
...
plot_sample_data <- function(data, sample_num = 1, t_values) {
  ...
  plot <- ggplot(plot_data, aes(x = t, y = value, color = type, linetype = type)) +
    ...
    theme_minimal() +
    scale_x_continuous(
      breaks = seq(min(t_values), max(t_values), length.out = 5),  # Adjust based on the data range
      labels = seq(0, 24, by = 6)  # Display labels from 0 to 24 at intervals of 2
    )+
...
plot_covariance_matrix <- function(Sigma, title, x = "t" , y = "t'") {
  ...
  plot <- ggplot(Sigma_melted, aes(Var1, Var2, fill = value)) +
    ...
    scale_x_continuous(
      breaks = seq(min(Sigma_melted$Var1), max(Sigma_melted$Var1), length.out = 5), # 13 evenly spaced ticks
      labels = seq(0, 24, by = 6)  # Labels at intervals of 2 (0, 2, 4, ..., 24)
    ) +
    scale_y_continuous(
      breaks = seq(min(Sigma_melted$Var2), max(Sigma_melted$Var2), length.out = 5), # 13 evenly spaced ticks
      labels = seq(0, 24, by = 6)  # Labels at intervals of 2 (0, 2, 4, ..., 24)
    ) +
